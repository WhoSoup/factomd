...
func isConnected(c net.Conn) bool {
	one := []byte{}
...
func Test_limitListenerSources_Accept(t *testing.T) {
	tests := []struct {
		name    string
		l       *limitListenerSources
		want    net.Conn
		wantErr bool
	}{
		// TODO: Add test cases.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.l.Accept()
			if (err != nil) != tt.wantErr {
				t.Errorf("limitListenerSources.Accept() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("limitListenerSources.Accept() = %v, want %v", got, tt.want)
			}
		})
	}
}

func Test_limitListenerAll_Accept(t *testing.T) {
